...
<div class="row">
    <div class="col-8">
        ...
        <my-component name="Piotr" id="2" is-enabled="true">
        </my-component>
        ...
        <comment-form url="{{ route('posts.comments.store', ['post' => $post->id]) }}"
            submit-label="{{ __('Add comment') }}" :user="{{ Auth::user() }}" :post={{ $post->id }}>
        </comment-form>
        @else
        ...
        <comment-list :post="{{ $post->id }}" post-id="{{ $post->id }}"></comment-list>
    </div>
    <div class="col-4">
